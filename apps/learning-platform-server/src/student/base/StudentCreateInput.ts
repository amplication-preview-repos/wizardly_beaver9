/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CertificateCreateNestedManyWithoutStudentsInput } from "./CertificateCreateNestedManyWithoutStudentsInput";
import {
  ValidateNested,
  IsOptional,
  IsString,
  MaxLength,
} from "class-validator";
import { Type } from "class-transformer";
import { CompletionCreateNestedManyWithoutStudentsInput } from "./CompletionCreateNestedManyWithoutStudentsInput";
import { IsJSONValue } from "../../validators";
import { GraphQLJSON } from "graphql-type-json";
import { InputJsonValue } from "../../types";

@InputType()
class StudentCreateInput {
  @ApiProperty({
    required: false,
    type: () => CertificateCreateNestedManyWithoutStudentsInput,
  })
  @ValidateNested()
  @Type(() => CertificateCreateNestedManyWithoutStudentsInput)
  @IsOptional()
  @Field(() => CertificateCreateNestedManyWithoutStudentsInput, {
    nullable: true,
  })
  certificates?: CertificateCreateNestedManyWithoutStudentsInput;

  @ApiProperty({
    required: false,
    type: () => CompletionCreateNestedManyWithoutStudentsInput,
  })
  @ValidateNested()
  @Type(() => CompletionCreateNestedManyWithoutStudentsInput)
  @IsOptional()
  @Field(() => CompletionCreateNestedManyWithoutStudentsInput, {
    nullable: true,
  })
  completions?: CompletionCreateNestedManyWithoutStudentsInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  email?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
  })
  @IsJSONValue()
  @IsOptional()
  @Field(() => GraphQLJSON, {
    nullable: true,
  })
  profilePicture?: InputJsonValue;
}

export { StudentCreateInput as StudentCreateInput };
